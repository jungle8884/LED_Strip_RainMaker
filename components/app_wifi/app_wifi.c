 ...
 * @brief 启动应用程序WiFi
 *
 * 根据给定的弹出类型，启动应用程序WiFi并配置WiFi Provisioning Manager。
 *
 * @param pop_type 弹出类型
 *
 * @return 返回错误码，表示函数执行结果
 */
esp_err_t app_wifi_start(app_wifi_pop_type_t pop_type)
{
    /* 配置 Provisioning Manager: Provisioning Manager 
    是用来管理设备配置（例如 Wi-Fi 凭据）的组件。
    它有多种配置方案，如通过 SoftAP（软件接入点）或 BLE（蓝牙低功耗） */
    wifi_prov_mgr_config_t config = {
        /* 这部分代码使用预处理指令来
        根据编译时配置选择 Provisioning Scheme。
        如果定义了 CONFIG_APP_WIFI_PROV_TRANSPORT_BLE，
        则使用 BLE 作为配置方案；否则，使用 SoftAP。 */
#ifdef CONFIG_APP_WIFI_PROV_TRANSPORT_BLE
        ...
        /* 这部分代码也为 Provisioning Manager 选择一个特定于方案的事件处理器。
        对于 BLE 方案，它选择一个用于释放与 BLE 相关的内存的事件处理器；
        对于 SoftAP 方案，则选择没有事件处理器（WIFI_PROV_EVENT_HANDLER_NONE）。*/
#ifdef CONFIG_APP_WIFI_PROV_TRANSPORT_BLE
    ...
    // 定义了一个布尔变量provisioned，用于标识设备是否已配置（即是否已经连接到WiFi网络）
    bool provisioned = false; 
    /* 调用wifi_prov_mgr_is_provisioned函数检查设备是否已配置，并将结果存储在provisioned变量中。 */
    wifi_prov_mgr_is_provisioned(&provisioned);
    /* 开始配置服务 如果设备尚未配置（!provisioned为真），则执行以下步骤 */
    if (!provisioned) {
        ESP_LOGI(TAG, "Starting provisioning");
#ifdef ESP_NETIF_SUPPORTED
#if CONFIG_ESP_WIFI_SOFTAP_SUPPORT
        esp_netif_create_default_wifi_ap(); // 根据编译选项，如果支持ESP网络接口和WiFi软接入点（SoftAP），则创建一个默认的WiFi接入点。
#endif
#endif
        ...
        char *service_name = "ESP_Friends"; // 修改蓝牙名称
    

        /* What is the service key (Wi-Fi password)
         * NULL = Open network
         * This is ignored when scheme is wifi_prov_scheme_ble
         * 这里设置了服务的名称（WiFi SSID或BLE设备名称）和服务密钥（WiFi密码）。
         * 服务密钥设置为NULL，意味着如果使用WiFi SoftAP方案，WiFi网络将是开放的。
         */
        ...
        //     return ESP_ERR_NO_MEM;
        // }
        // 定义了安全级别（使用安全通信）和Proof of Possession（POP，用于验证设备的所有权）
        char *pop = "1234567";
...
#endif
#endif /* CONFIG_APP_WIFI_PROV_TRANSPORT_BLE 根据使用的传输方案（BLE或SoftAP），打印相应的QR码，以便用户可以使用手机扫描并连接到设备。*/
        ...
        // 自动申请密钥的函数在heap上申请了内存，所以配网开始之后需要释放申请的内存
        // if (pop) {
        //     free(pop);
        // }
        // 手动申请密钥的话就不需要在heap上申请内存，也不需释放资源了，所以要删除掉这段释放资源的代码
